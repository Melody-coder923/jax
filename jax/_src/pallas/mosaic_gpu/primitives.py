...
def _barrier_arrive_abstract_eval(barrier, *args, **params):
  del args, params  # Unused.
  _check_ref(barrier, "barrier", gpu_core.SMEM)
  if barrier.inner_aval.dtype.for_tensor_core:
...
def barrier_wait(barrier: pallas_core.AbstractMemoryRef) -> None:
  ...
  for_tensor_core = barrier.inner_aval.dtype.for_tensor_core
  flat_transforms, transforms_treedef = tree_util.tree_flatten(transforms)
  barrier_wait_p.bind(
      barrier, *flat_transforms, transforms_treedef=transforms_treedef,
      for_tensor_core=for_tensor_core,
  )
...
def tcgen05_mma(acc: gpu_core.AbstractMemoryRef,
                a: gpu_core.AbstractMemoryRef,
                b: gpu_core.AbstractMemoryRef,
                barrier: gpu_core.AbstractMemoryRef,
  ...
  Args:
    ...
    collective: Whether the MMA is being executed on paired blocks. The MMA will
      only be issued by one block between the pair.
  """
  if isinstance(a, pallas_core.TransformedRef):
    a_transforms_leaves, a_transforms_tree = jax.tree.flatten(a.transforms)
    a = a.ref
  ...
  tcgen05_mma_p.bind(acc, a, b, barrier,
...
def _tcgen05_mma_abstract_eval(acc, a, b, barrier,
                               *transforms_leaves,
                               a_transforms_tree, b_transforms_tree,
                               accumulate,
                               collective):
  del transforms_leaves, a_transforms_tree, b_transforms_tree, collective
  if acc.memory_space != gpu_core.GPUMemorySpace.TMEM:
    raise ValueError("Accumulator must be a TMEM Ref.")
  if a.memory_space not in (gpu_core.GPUMemorySpace.TMEM,
                            gpu_core.GPUMemorySpace.SMEM):
    raise ValueError("LHS must be a TMEM or SMEM Ref.")
  if b.memory_space != gpu_core.GPUMemorySpace.SMEM:
    raise ValueError("RHS must be a SMEM Ref.")
  if not barrier.inner_aval.dtype.for_tensor_core:
    raise ValueError("MMA barrier must have for_tensor_core set to True.")
  return []

@lowering.register_lowering_rule(
    tcgen05_mma_p, *gpu_core.LANExWARP_SEMANTICS)
def _tcgen05_mma_lowering(
  ...
  if a_transforms_tree is not None:
    ...
    a_ref, a_transforms = lowering._handle_transforms(
        a_ref, a_transforms, handle_transposes=True, handle_reshapes=False
    )
    match a_transforms:
    ...
    if lhs_tiling != (8, swizzle_elems):
      raise NotImplementedError("MMA lhs tiling does not fit swizzle")
  else:
    b_transforms_leaves = transforms_leaves  # type: ignore

  b_transforms = b_transforms_tree.unflatten(b_transforms_leaves)
  b_ref, b_transforms = lowering._handle_transforms(
      b_ref, b_transforms, handle_transposes=True, handle_reshapes=False
  )
  match b_transforms:
    case (gpu_core.UnswizzleRef(rhs_swizzle), gpu_core.UntileRef(rhs_tiling)):
      rhs_transpose = False
  ...
  if lhs_swizzle is not None:
    swizzle_elems = rhs_swizzle // a_aval.dtype.itemsize
    if rhs_swizzle != lhs_swizzle:
      raise NotImplementedError("MMA rhs swizzle must match lhs swizzle")
    if rhs_tiling != (8, swizzle_elems):
      raise NotImplementedError("MMA rhs tiling does not fit swizzle")
  if lhs_transpose or rhs_transpose:
    raise NotImplementedError("MMA does not support transpose")
