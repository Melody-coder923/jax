...
def _infer_multi_dim_reduction_op_layout(
  ...
  if in_layout:
    ...
    if reduction_dims == [0]:
      ...
      raise NotImplementedError(
          "Only a single reducation dimension is supported, but got:"
          f" {reduction_dims}"
      )
    return [in_layout, out_layout], [out_layout]

  # The source does not have a layout set. Find out the layout of the result
  # if possible. We can do this either from consumers or from the acc input.
  ...
  user_layout = None
  if user_layouts:
    user_layout = _choose_representative_layout(user_layouts)
  ...
  if user_layout is None and acc_layout is None:
    return None
  elif user_layout is None:
    out_layout = acc_layout
  elif acc_layout is None:
    out_layout = user_layout
  else:
    if acc_layout != user_layout:
      raise ValueError(
          "Acc layout does not match user layout: {acc_layout} vs {user_layout}"
      )
    out_layout = user_layout

  if out_layout != wgmma_row_layout and out_layout != wgmma_col_layout:
    raise NotImplementedError(f"Unsupported out layout: {out_layout}")
